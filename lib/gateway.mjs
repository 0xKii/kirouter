...
try {
  const _pkgCandidates = [
    pathResolve(__dirname, '..', 'package.json'),  // npm install: lib/gateway.mjs → ../package.json
    pathResolve(__dirname, 'package.json'),         // runtime flat: gateway.mjs + package.json same dir
  ];
  for (const _p of _pkgCandidates) {
    try {
      const _v = JSON.parse(readFileSync(_p, 'utf8')).version;
      if (_v) { _pkgVer = _v; break; }
    } catch (e) { /* try next */ }
  }
} catch (e) { /* fall back below */ }
const GATEWAY_VERSION = process.env.KI_GATEWAY_VERSION || ('v' + _pkgVer);
