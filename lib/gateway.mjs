...
function isCreditsExhaustedError(status, text) {
  ...
  if (t.includes('credits exhausted') || t.includes('credit exhausted') || t.includes('insufficient credit') || t.includes('insufficient_quota')) return true;
  return false;
}
...
const GATEWAY_KEY = loadOrCreateGatewayKey();
const GATEWAY_VERSION = process.env.KI_GATEWAY_VERSION || 'v1.0.5';
  ...
  try {
    ...
    if (path.startsWith('/admin/')) {
      ...
      if (path === '/admin/update-check' && req.method === 'GET') {
        const now = Date.now();
        if (UPDATE_CHECK_CACHE && (now - UPDATE_CHECK_CACHE.checkedAt) < 3600_000) {
        ...
        try {
          ...
          if (!r.ok) return send(res, 200, { current: GATEWAY_VERSION, latest: null, updateAvailable: false, error: 'registry_unavailable', checkedAt: new Date().toISOString() });
          const data = await r.json();
          const latest = data['dist-tags']?.latest || null;
          const current = GATEWAY_VERSION.replace(/^v/, '');
          ...
          return send(res, 200, { current: GATEWAY_VERSION.replace(/^v/, ''), latest: null, updateAvailable: false, error: sanitizeErrorText(e.message, 100), checkedAt: new Date().toISOString() });
        }
      }
