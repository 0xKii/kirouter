...
function redactedBackup() {
  ...
  return {
    ...
    gateway: { version: process.env.KI_GATEWAY_VERSION || 'v1.0.4' },
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (path === '/health') return send(res, 200, {
      status: 'ok',
      service: 'ki-gateway',
      version: process.env.KI_GATEWAY_VERSION || 'v1.0.4',
    ...
    if (path.startsWith('/admin/')) {
      ...
      if (path === '/admin/summary') {
        ...
        return send(res, 200, {
          ...
          models,
          clientKeys,
          gateway: { version: process.env.KI_GATEWAY_VERSION || 'v1.0.4', providerCount: Object.keys(PROVIDERS).length },
        });
      }
