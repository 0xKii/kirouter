...
async function callProviderChat(provider, upstreamModel, payload, opts = {}) {
  ...
  for (let attempt = 0; attempt <= RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      if (isCodebuddyProvider(provider) && !r?.ok) {
        ...
        if (retryable && attempt < RETRY_ATTEMPTS && r.__cbKeyId) {
          recordProviderKeyUsage(r.__cbPrefix || provider.__prefix || '', r.__cbKeyId, {
      ...
      if (attempt < RETRY_ATTEMPTS && retryable) {
        if (isCodebuddyProvider(provider) && !hasEligibleProviderKey(provider, provider.__prefix || '')) return r;
